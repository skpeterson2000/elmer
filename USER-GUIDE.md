...
- **Say ELMER in code when the program opens.** Five characters at twenty words a minute on 1020 Hz, once when you open ELMER rather than once a page. It is the station identifying itself, and it is a plain way of hearing that the unit is up and the sound works before you go hunting for a volume control. Turn it off where it would not be welcome: a net in progress, a classroom, a field site at night. Being quiet is sometimes what being in a community asks of a station. On a browser that keeps audio silent until a page has been touched, it waits for your first click and goes then.
...
What the law allows, and what convention puts where. Pick the **licence class** at the top, or enter your callsign in the strip below it and ELMER uses your actual privileges and tells you when the licence expires. The page opens on the class you hold every time, whatever you were reading last. The picker changes only what is on the screen: reading a class above your own is the point of having it, and it is how you decide whether the upgrade is worth sitting for, so a class you do not hold brings a note saying so rather than a locked door. What it is not is a claim. It does not tell the rest of ELMER that you hold that class, and it does not open a study pool - the pools follow your licence, and your licence is set with your callsign or on the setup page. Each band is a button; the chosen band shows its bar coloured by activity, with the parts your class may not transmit on hatched out, the privileges for your class beside the rule that grants them, and a table of what happens where and whether you may use it in that mode.
...
- **Learn** is the Koch method: a lesson slider from two characters to forty, the lesson's characters drawn as shapes to compare against, and two ways to work through them. Below them is a grid of where you stand, green for copied reliably, amber for shaky, red for needs work, grey for not met yet. Hover a character for what it was confused with, and how often it had to be sent again.

  **Learn them one at a time** is the place to start, and there is no clock on it. One character sounds and then nothing happens at all until you answer. Compare what you heard against the shapes above and pick the one you think it was, by clicking it or by typing it; ask for it **Again** as often as you like first.
  ...
  Nothing here is timed and nothing is scored, because telling two sounds apart is a different skill from copying at speed, and everybody takes a different length of time over it. The drills upstairs keep the numbers; this pane keeps none, and the colouring stops once you move on to them.
