...
When a unit stops being one person's, ELMER offers a password once, and a decline stays declined.
...
This guide is on the shelf with your manuals, indexed and searched like any book and opened in the same reader, so a kiosk with no file manager still has it. It is built from the text in the docs folder when ELMER starts, and rebuilt when the text changes with an update.
...
ELMER checks the repository it was installed from and tells you what it finds, on the dashboard, with the waiting changes listed by subject. It never applies an update on its own. **Update now** is always your press, whenever it suits you, and updating is a fast-forward that never happens while there are local changes on the unit. A copy that was downloaded as a zip rather than cloned has no link back and cannot update itself; the Windows quickstart says how to give it one.

### Report a problem

Three presses, and nothing leaves on the first two. **Report a problem** writes a file with the versions, the recent errors and the tail of the log, with your callsign, QTH and network addresses taken out, and shows you where it is. **Read it before you send it** opens the text. **Send it** sends what you just read, by the drop, a public address that only takes reports in, or through your own mail server if you have set one up under **Mail home**. If it cannot be sent, the page says how to get it there by hand.
...
- **A Fix, Update now, Report a problem or Mail home is not offered:** they are offered only to a browser on the unit itself, never over the network.

Anything else: press **Report a problem**, read what it wrote, and send it.
